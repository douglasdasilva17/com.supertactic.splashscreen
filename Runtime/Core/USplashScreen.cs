...
namespace Supertactic.SplashScreen
{
    public class USplashScreen : MonoBehaviour
    {
        ...
        private Color _startBackColor = Color.black;
        /// <summary>
        ...
        public Text BrandDescription = null;
        public Color BrandTextColor = Color.white;
        ...
        void Start()
        {
            ...
            if (BackImage != null)
            {
                _startBackColor = BackImage.color;
                BackImage.color = BackColor;
            }

            if (ShowBrands)
            {
                BrandText.color = BrandTextColor;
                BrandDescription.color = BrandTextColor;
            }
            ...
            //Desactive all splash, for show on order and set the sounds to mute (if needed).
            for (int i = 0; i < m_SplashScreens.Count; i++)
            {                
                m_SplashScreens[i].SplashUI.SetActive(false);
                m_SplashScreens[i].SplashUI.GetComponent<USplash>().Mute(m_SplashScreens[i].Mute);
            }
            InstanceBrands();
        ...
        void OnDisable()
        {
            if (BackImage != null)
            {
                BackImage.color = _startBackColor;                
            }
        }
